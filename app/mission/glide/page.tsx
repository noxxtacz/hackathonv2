...
export default function GlidePage() {
  ...
  useEffect(() => {
    ...
    if (session?.analysis?.stressLevel) {
      ...
      console.log('[GlidePage] Stress:', stressLevel, 'Difficulty:', diff);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const initGame = async () => {
      ...
      const { GlideScene } = await import('@/game/glide/GlideScene');
      ...
      if (!listenerAttachedRef.current) {
        game.events.on('mission:complete', (payload: GameResult) => {
          console.log('[GlidePage] ========== RECEIVED PAYLOAD ==========');
          console.log('[GlidePage] Raw payload:', JSON.stringify(payload));
          
          setGameResult({
            type: payload.type || 'glide',
            score: payload.score,
          ...
          console.log('[GlidePage] Saving game data:', gameData);
      ...
      (window as any).onGameEnd = (score: number, calmScore: number, durationSec: number) => {
        game.events.emit('mission:complete', { type: 'glide', score, calmScore, durationSec });
      };
    };
  ...
  const submitFeedback = () => {
    if (gameResult) {
      ...
      console.log('[GlidePage] submitFeedback - saving:', finalData);
      updateSessionWithGameData(finalData);
      
      // Verify the save
      const saved = getLatestSession();
      console.log('[GlidePage] Session after save:', saved?.game);
    }
    ...
    return (
      <div className="page-container">
        <div className="feedback-card">
          ...
          <h1 className="feedback-title">Peaceful Glide!</h1>
          
          <div className="feedback-stats">
            <div className="feedback-stat">
              <span className="feedback-stat-label">Lanterns</span>
  ...
  return (
    <div className="page-container">
      <GameFrame
        title="🌙 Glide"
        subtitle="Use arrow keys to guide the cloud. Collect lanterns. Press P to pause."
        showMode
        mode={difficulty}
      >
